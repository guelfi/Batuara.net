...
apply_migration "20260916010000_RemoveViewerAddUserHouseMember" \
    "UPDATE batuara.users SET role = 2 WHERE role = 3; UPDATE batuara.users SET role = 3 WHERE role = 4; ALTER TABLE batuara.users ADD COLUMN IF NOT EXISTS house_member_id integer NULL; UPDATE batuara.users u SET house_member_id = hm.\"Id\" FROM batuara.\"HouseMembers\" hm WHERE u.house_member_id IS NULL AND hm.\"Email\" IS NOT NULL AND lower(u.email) = lower(hm.\"Email\"); UPDATE batuara.users SET house_member_id = 57 WHERE house_member_id IS NULL AND lower(email) IN ('guelfi@msn.com', 'admin@batuara.org.br') AND EXISTS (SELECT 1 FROM batuara.\"HouseMembers\" WHERE \"Id\" = 57); UPDATE batuara.users SET house_member_id = 103 WHERE house_member_id IS NULL AND lower(email) = 'douglas@batuara.org.br' AND EXISTS (SELECT 1 FROM batuara.\"HouseMembers\" WHERE \"Id\" = 103); UPDATE batuara.users SET house_member_id = 68 WHERE house_member_id IS NULL AND lower(email) = 'madalena@batuara.org.br' AND EXISTS (SELECT 1 FROM batuara.\"HouseMembers\" WHERE \"Id\" = 68); UPDATE batuara.users SET house_member_id = 114 WHERE house_member_id IS NULL AND lower(email) = 'pepe@batuara.org.br' AND EXISTS (SELECT 1 FROM batuara.\"HouseMembers\" WHERE \"Id\" = 114); UPDATE batuara.users SET house_member_id = 30 WHERE house_member_id IS NULL AND lower(email) IN ('thaisy@batuara.org.br', 'taisy@batuara.org.br') AND EXISTS (SELECT 1 FROM batuara.\"HouseMembers\" WHERE \"Id\" = 30); UPDATE batuara.users SET house_member_id = 49 WHERE house_member_id IS NULL AND lower(email) = 'carol@batuara.org.br' AND EXISTS (SELECT 1 FROM batuara.\"HouseMembers\" WHERE \"Id\" = 49); CREATE INDEX IF NOT EXISTS \"IX_users_house_member_id\" ON batuara.users (house_member_id); DO \$\$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'FK_users_HouseMembers_house_member_id') THEN ALTER TABLE batuara.users ADD CONSTRAINT \"FK_users_HouseMembers_house_member_id\" FOREIGN KEY (house_member_id) REFERENCES batuara.\"HouseMembers\" (\"Id\") ON DELETE RESTRICT; END IF; END \$\$;"

apply_migration "20260916020000_AddContentVisibilityFlags" \
    "ALTER TABLE batuara.\"SiteSettings\" ADD COLUMN IF NOT EXISTS \"OrixasVisibility\" integer NOT NULL DEFAULT 0; ALTER TABLE batuara.\"SiteSettings\" ADD COLUMN IF NOT EXISTS \"GuidesVisibility\" integer NOT NULL DEFAULT 0; ALTER TABLE batuara.\"SiteSettings\" ADD COLUMN IF NOT EXISTS \"UmbandaLinesVisibility\" integer NOT NULL DEFAULT 0; ALTER TABLE batuara.\"SiteSettings\" ADD COLUMN IF NOT EXISTS \"PrayersVisibility\" integer NOT NULL DEFAULT 0;"
