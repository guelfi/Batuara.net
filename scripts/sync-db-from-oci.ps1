...
$BackupFile = "$env:TEMP\batuara_sync.sql.gz"
...
if (-not (Test-Path $SshKey)) { Write-Fail "Chave SSH nao encontrada: $SshKey" }
$running = docker ps --format "{{.Names}}" 2>&1 | Select-String "^$LocalContainer$"
if (-not $running) { Write-Fail "Container '$LocalContainer' nao esta em execucao." }
Write-Ok "Pre-requisitos OK"

# 2. Testar SSH
Write-Step "Testando conexao SSH com OCI..."
ssh -i $SshKey -o StrictHostKeyChecking=no -o ConnectTimeout=10 "${OciUser}@${OciHost}" "echo SSH_OK" | Out-Null
if ($LASTEXITCODE -ne 0) { Write-Fail "Falha na conexao SSH" }
Write-Ok "Conexao SSH OK"

# 3. Backup na OCI
Write-Step "Gerando backup na OCI (schema: $DbSchema)..."
$remoteFile = "/tmp/batuara_sync.sql.gz"
ssh -i $SshKey -o StrictHostKeyChecking=no "${OciUser}@${OciHost}" `
    "docker exec $OciContainer pg_dump -U $DbUser -d $DbName --schema=$DbSchema --no-owner --no-acl | gzip > $remoteFile"
if ($LASTEXITCODE -ne 0) { Write-Fail "Falha ao gerar backup na OCI" }
Write-Ok "Backup gerado no servidor OCI"

# 4. Download
Write-Step "Baixando backup..."
scp -i $SshKey -o StrictHostKeyChecking=no "${OciUser}@${OciHost}:${remoteFile}" $BackupFile
if ($LASTEXITCODE -ne 0) { Write-Fail "Falha ao baixar backup" }
$sizeKB = [math]::Round((Get-Item $BackupFile).Length / 1KB, 1)
Write-Ok "Backup local: $BackupFile (${sizeKB} KB)"
ssh -i $SshKey -o StrictHostKeyChecking=no "${OciUser}@${OciHost}" "rm -f $remoteFile" 2>&1 | Out-Null

# 5. Restore local
Write-Step "Restaurando banco local..."
...
docker exec $LocalContainer bash -c "gunzip -c /tmp/restore.sql.gz | psql -U $DbUser -d $DbName" 2>&1 | Out-Null
...
$tabCount = docker exec $LocalContainer psql -U $DbUser -d $DbName -t -c `
    "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema='$DbSchema' AND table_type='BASE TABLE';"
Write-Ok "Tabelas no schema '$DbSchema': $($tabCount.Trim())"
...
Write-Host "  Reinicie a API local:                    " -ForegroundColor Green
